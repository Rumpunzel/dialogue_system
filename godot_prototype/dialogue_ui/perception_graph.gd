...
func _process(_delta):
	update_perceptions_graph(get_parent().calculate_perception_value(get_parent().character_perceptions.get(get_node("/root/main/Player"), [[]])[NPC.PERCEPTION_VALUES]))

func _draw():
	#print(get_approval_rating_graph())
...
func get_graph(new_perceptions):
	...
	for i in range(points_amount):
		points.append(center + Vector2(0, -get_rect().size.y / 4 * (1 + new_perceptions[i])).rotated((i / float(points_amount)) * TAU))
