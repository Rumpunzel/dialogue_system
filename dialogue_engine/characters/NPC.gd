...
func calculate_approval_rating(target, print_update = false):
	...
	if not character_perceptions.get(target) == null:
		...
		for value in perception_values.keys():
			var approval_change = ((perception_values[value] / GAME_CONSTANTS.MAX_PERCEPTION_VALUE) * calculated_personal_values[value]) * (1.0 / GAME_CONSTANTS.PERCEPTION_VALUES.size()) * GAME_CONSTANTS.MAX_APPROVAL_VALUE
